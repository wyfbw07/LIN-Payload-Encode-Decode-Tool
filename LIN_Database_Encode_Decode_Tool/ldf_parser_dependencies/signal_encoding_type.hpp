...
public:
	...
	int getOffsetFromPhysicalValue(double physicalValue) const { return std::get<0>(getTypeInfoFromPhysicalValue(physicalValue))[3]; }
	int getFactorFromPhysicalValue(double physicalValue) const { return std::get<0>(getTypeInfoFromPhysicalValue(physicalValue))[2]; }
...
private:
	...
	std::tuple<std::array<int, 4>, std::string, ValueType> getTypeInfoFromPhysicalValue(double& physicalValue) const;
